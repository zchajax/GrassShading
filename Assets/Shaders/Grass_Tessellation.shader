Shader "Custom/Grass_Tessellation"
{
	Properties
	{
		_GrassHeight("Grass height", float) = 0
        _GrassWidth("Grass width", float) = 0.005
        _GrassOffset("Grass Blades Offset", Range(0, 0.5)) = 0.05

		_ShakeStrength("Shake Strength", Range(0, 0.05)) = 0.01
        _ShakeFrequency("Shake Frequency", Range(0, 100)) = 10

		_Tessellation("Tessellation", Range(0, 30)) = 1
		_MinimunTessellation("Minimum Tessellation", Range(0, 15)) = 1
		...
		SubShader
	{
		CGINCLUDE
			...
			void geom(triangle v2g input[3], inout TriangleStream<g2f> triStream)
			{
				...
				for (int i = 0; i < 1; i++)
				{
					float r1 = random2(mul(unity_ObjectToWorld, input[0].vertex).xz);
					float r2 = random2(mul(unity_ObjectToWorld, input[1].vertex).xz);
            ...
            float TessellationEdgeFactor (v2g cp0, v2g cp1)
            {
                ...
                float factor = lerp(_Tessellation, _MinimunTessellation, saturate(distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, cp0.vertex)) / _MaxCameraDistance));
                return factor;
            }
            ...
            TessellationFactors PatchConstantFunction(InputPatch<v2g, 3> patch)
            {
                ...
                float f0 = TessellationEdgeFactor(patch[1], patch[2]);
                float f1 = TessellationEdgeFactor(patch[2], patch[0]);
                float f2 = TessellationEdgeFactor(patch[0], patch[1]);
                float average = (f0 + f1 + f2) * (1 / 3.0);
